...
### 视图×2
- `v_algorithm_ranking` — 算法综合排名（RANK() OVER 按性能耗时升序）
- `v_user_activity` — 用户活跃度汇总（教学次数+性能批次+最后活动时间）

### 存储过程×1
- `sp_user_report(IN p_user_id VARCHAR(32))` — 生成用户综合实验报告（3个结果集：摘要+常用算法+分算法统计）
...
-- 调用存储过程
CALL sp_user_report('a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6');
```
